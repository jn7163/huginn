...
module Agents
  ...
  class HttpStatusAgent < Agent
    ...
    description <<-MD
      ...
      Specify a `url` and the HttpStatusAgent will produce an event with the HTTP status code. If you specify one or more `Headers to save` (comma-delimited) as well, that header or headers' value(s) will be included in the event.
